<x-layouts.app.base :title="'Add Room Type'">
    <x-slot name="header">
        <x-staff.header
    ...
    <section class="bg-base-200 py-6">
        <div class="mx-auto max-w-4xl px-4 space-y-4">
            <div class="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
                <div>
                    <h1 class="text-2xl font-bold">Add room type</h1>
                    <p class="text-sm text-base-content/70">Create a new room type for {{ $hotelName ?? 'your hotel' }}.</p>
                </div>
            </div>
            ...
            <div class="card bg-base-100 shadow">
                <div class="card-body space-y-4">
                    ...
                    <p class="text-sm text-base-content/70">Set capacity, base occupancy, and nightly prices. Choose which rate is active right now.</p>

                    <form method="POST" action="{{ route('staff.manager.room-types.store') }}" class="space-y-4">
                        @csrf
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            ...
                            <div>
                                ...
                                <input id="base_occupancy" name="base_occupancy" type="number" min="1" max="40" value="{{ old('base_occupancy', 1) }}" class="input input-bordered w-full" required>
                        ...
                        <div class="grid gap-4 md:grid-cols-3">
                            <div>
                                ...
                                <input id="max_adults" name="max_adults" type="number" min="1" max="40" value="{{ old('max_adults', 2) }}" class="input input-bordered w-full" required>
                                @error('max_adults')
                                    <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <input id="max_children" name="max_children" type="number" min="0" max="20" value="{{ old('max_children', 0) }}" class="input input-bordered w-full" required>
                                @error('max_children')
                                    <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                @enderror
                            </div>
                            <div>
                                ...
                                <select id="active_rate" name="active_rate" class="select select-bordered w-full" required>
                                    <option value="off_peak" @selected(old('active_rate', 'off_peak') === 'off_peak')>Off-peak price active</option>
                                    <option value="peak" @selected(old('active_rate') === 'peak')>Peak price active</option>
                                </select>
                                @error('active_rate')
                                    <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                @enderror
                        ...
                        <div class="grid gap-4 md:grid-cols-2">
                            <div>
                                ...
                                <div class="join w-full">
                                    ...
                                    <input id="price_off_peak" name="price_off_peak" type="number" step="0.01" min="0" value="{{ old('price_off_peak') }}" class="join-item input input-bordered w-full" required>
                                </div>
                            ...
                            <div>
                                ...
                                <div class="join w-full">
                                    <span class="join-item btn btn-ghost">£</span>
                                    <input id="price_peak" name="price_peak" type="number" step="0.01" min="0" value="{{ old('price_peak') }}" class="join-item input input-bordered w-full" required>
                                </div>
                                @error('price_peak')
                                    <p class="mt-1 text-sm text-error">{{ $message }}</p>
                                @enderror
